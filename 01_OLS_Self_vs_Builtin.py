...
print("\n" + "="*80)
print("ORDINARY LEAST SQUARES (OLS) - COMPARISON")
print("="*80)

# Fit Self-Constructed OLS
ols_self = OLS_Self()
ols_self.fit(X, y)
y_pred_self = ols_self.predict(X)

print("\n[SELF-CONSTRUCTED OLS]")
print(f"Intercept (β₀): {ols_self.intercept:.6f}")
print(f"Slope (β₁):     {ols_self.slope:.6f}")
print(f"R²:             {r2_score(y, y_pred_self):.6f}")
print(f"MSE:            {mean_squared_error(y, y_pred_self):.6f}")
print(f"Equation:       y = {ols_self.intercept:.4f} + {ols_self.slope:.4f}x")


ols_sklearn = LinearRegression()
ols_sklearn.fit(X, y)
y_pred_sklearn = ols_sklearn.predict(X)

print("\n[BUILT-IN OLS - sklearn.LinearRegression]")
print(f"Intercept (β₀): {ols_sklearn.intercept_:.6f}")
print(f"Slope (β₁):     {ols_sklearn.coef_[0]:.6f}")
print(f"R²:             {r2_score(y, y_pred_sklearn):.6f}")
print(f"MSE:            {mean_squared_error(y, y_pred_sklearn):.6f}")
print(f"Equation:       y = {ols_sklearn.intercept_:.4f} + {ols_sklearn.coef_[0]:.4f}x")
...
print("-"*80)
print(f"{'Intercept':<20} {ols_self.intercept:<20.10f} {ols_sklearn.intercept_:<20.10f} {abs(ols_self.intercept - ols_sklearn.intercept_):<15.10f}")
print(f"{'Slope':<20} {ols_self.slope:<20.10f} {ols_sklearn.coef_[0]:<20.10f} {abs(ols_self.slope - ols_sklearn.coef_[0]):<15.10f}")
...
ax.set_xlabel('X')
ax.set_ylabel('y')
ax.set_title('OLS Regression: Self vs Built-in')
ax.legend()
...
plt.tight_layout()
plt.savefig('ols_comparison.png', dpi=300, bbox_inches='tight')
plt.show()
...
coef_diff = abs(ols_self.slope - ols_sklearn.coef_[0])
intercept_diff = abs(ols_self.intercept - ols_sklearn.intercept_)
...
print("\n" + "="*80)
print("CONCLUSION: Self-constructed and built-in OLS produce IDENTICAL results")
